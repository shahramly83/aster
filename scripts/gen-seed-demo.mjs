...
--
-- HOW TO USE
--   Paste this whole file into the Supabase SQL editor and run it. It targets the
--   'onlazy' test workspace; change v_slug below to seed a different one.
--
-- Runs as a single do-block, so it works in the Supabase SQL editor as-is: no psql
-- meta-commands (\\set and friends are not understood there). The block raises on a
-- missing workspace, which stops it just as ON_ERROR_STOP would.
--
...
do $seed$
declare
  v_slug     text := 'onlazy';   -- the target workspace's subdomain slug
  v_company  uuid;
  v_owner    uuid;
begin
