...
| ~~S5~~ | Medium | ~~`parse-application` is public, unmetered and unthrottled. Anyone with a public apply-page UUID can submit unlimited resumes, each firing several paid Claude calls.~~ | ✅ **FIXED (code)** — per-IP throttle (12 / 5 min, fallback 5) before any model work, via new `_shared/ratelimit.ts`. ⏳ Deploy `parse-application`. |
| ~~S6~~ | Medium | ~~`support-intake` is public with no rate limit, and sends email via Resend. Spam/quota-burn vector.~~ | ✅ **FIXED (code)** — per-IP throttle (6 / 5 min, fallback 3) before filing/emailing; RPC honeypot still applies. ⏳ Deploy `support-intake`. Turnstile still optional (needs a site/secret key). |
| ~~S7~~ | Low | ~~`_free_trial_used(text)` is PUBLIC-callable.~~ | ✅ **FIXED** — `0041` §1 applied. |
| ~~S8~~ | Low | ~~`marketing-chat`'s rate limiter fails open: if the DB is unreachable, throttling silently disables on a public Anthropic endpoint.~~ | ✅ **FIXED (code)** — now falls back to a per-isolate cap (10 / min) instead of fail-open, via `_shared/ratelimit.ts`. ⏳ Deploy `marketing-chat`. |
