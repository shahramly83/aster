-- ============================================================================
-- Aster — Growth tier + reconciled credit limits + See Why credit
-- ============================================================================
-- Brings the server-enforced limits in line with the locked pricing matrix.
-- Until now the DB knew only free / pro / enterprise, so it could not tell the
-- Free plan apart from Growth. This adds a real 'growth' tier and sets every
-- metered pool to its matrix value, per tier:
--
...
--   See Why               5       30      100     unlimited   (NEW, Option B:
--                                                              its own credit,
--                                                              charged per
--                                                              candidate, cached)
--
-- null limit = unlimited. All pools reset on the same rolling 30-day cycle from
-- signup (reusing _ai_rank_period). AI Rank / AI Insight / See Why are charged
-- once per candidate and cached; regenerating charges again (confirmed in UI).

-- ---------------------------------------------------------------------------
-- 1. Add the 'growth' tier. (Safe as a string literal in the function bodies
--    below; the enum value is only cast at execution time.)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Reconciled limit functions (matrix numbers, 4 tiers).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. See Why usage + charge RPCs (mirrors the AI Rank pattern in 0008).
--    A credit is charged once per candidate by the caller and cached; the
--    generic bump here advances the cycle counter unless the plan cap is hit.
-- ---------------------------------------------------------------------------
